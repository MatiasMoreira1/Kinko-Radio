...
// Play/Pause al hacer clic en el botón
...
function playNext() {
    // Verificar si un anuncio está sonando
    if (isAdPlaying) {
        isAdPlaying = false; // Termina el anuncio, reproducir canción
    ...
    // Verificar si corresponde reproducir un anuncio después de 3 canciones
    if (songCounter % 3 === 0 && !isAdPlaying) {
        const adIndex = Math.floor(Math.random() * ads.length);
        loadMedia(ads[adIndex]); // Cargar el anuncio
...
// Escuchar el fin del audio y reproducir el siguiente
